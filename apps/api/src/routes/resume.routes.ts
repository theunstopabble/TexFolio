...
router.get('/', async (req: Request, res: Response) => {
  try {
    const resumes = await Resume.find({ userId: (req as any).userId }).sort({ createdAt: -1 });
    res.status(200).json({
      success: true,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const resume = await Resume.findOne({ _id: id, userId: (req as any).userId });
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const resume = await Resume.findOneAndDelete({ _id: id, userId: (req as any).userId });
...
router.get('/:id/pdf', async (req: Request, res: Response) => {
  try {
    ...
    // Send PDF file
    res.download(pdfPath, `${resume.personalInfo.fullName}_Resume.pdf`, (err) => {
      if (err) {
